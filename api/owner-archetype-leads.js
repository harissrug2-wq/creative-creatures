...
export default async function handler(req,res){res.setHeader('Access-Control-Allow-Origin','*');res.setHeader('Access-Control-Allow-Methods','GET,POST,DELETE,OPTIONS');res.setHeader('Access-Control-Allow-Headers','Content-Type');if(req.method==='OPTIONS')return json(res,204,{});const c=cfg();if(!c)return json(res,503,{error:'Lead database is not configured.'});try{
 ...
 if(req.method==='GET'){
   ...
   if(all){
     if(!requireAdmin(req))return json(res,401,{error:'Admin authentication required.'});
     // Keep admin history readable across older and newer lead-table schemas.
     const rows=await db(c,'owner_archetype_leads?select=*&order=created_at.desc');
 ...
 if(req.method!=='POST')return json(res,405,{error:'Method not allowed.'});const b=typeof req.body==='string'?JSON.parse(req.body||'{}'):(req.body||{});const name=clean(b.name||`${b.firstName||''} ${b.lastName||''}`);const email=lower(b.email);const agencyUrl=clean(b.agencyUrl||b.agency_url);const normalized=normalizeUrl(agencyUrl);const requestedJourney=lower(b.journey).replace(/-/g,'_');const journey=['aofi_free','diagnostic','accelerator','platform','fractional_coo','owner_archetype'].includes(requestedJourney)?requestedJourney:'diagnostic';if(!name||!email||!normalized)return json(res,422,{error:'Name, email, and agency URL are required.'});if(!/^\S+@\S+\.\S+$/.test(email))return json(res,422,{error:'Enter a valid email address.'});
 ...
 const record={name,name_normalized:lower(name),email,email_normalized:email,agency_url:agencyUrl,agency_url_normalized:normalized,agency_name:clean(b.agencyName||b.agency_name)||deriveName(agencyUrl),journey,source:clean(b.source)||'owner-archetype',archetype_answers:b.archetypeAnswers||b.archetype_answers||{},archetype_result:b.archetypeResult||b.archetype_result||{},report_data:b.reportData||b.report_data||{},converted_account_id:null,converted_at:null,payment_completed_at:null,updated_at:new Date().toISOString()};
