...
const JIRA_SELECT='id,account_id,cloud_id,site_url,site_name,connected_account_id,connected_name,connected_email,access_token_encrypted,refresh_token_encrypted,access_token_expires_at,scopes,resources,status,last_synced_at,last_sync_error,created_at,updated_at';
...
function publicJiraConnection(row){return row?{connected:row.status==='connected',cloudId:row.cloud_id||'',siteUrl:row.site_url||'',siteName:row.site_name||'',connectedName:row.connected_name||'',connectedEmail:row.connected_email||'',scopes:Array.isArray(row.scopes)?row.scopes:[],resources:Array.isArray(row.resources)?row.resources:[],status:row.status||'connected',lastSyncedAt:row.last_synced_at||null,lastSyncError:row.last_sync_error||'',updatedAt:row.updated_at||null}:{connected:false,cloudId:'',siteUrl:'',siteName:'',connectedName:'',connectedEmail:'',scopes:[],resources:[],status:'disconnected',lastSyncedAt:null,lastSyncError:''}}
...
async function ensureJiraAccess(c,connection){const jc=jiraConfig();if(!jc)throw new Error('Jira is not configured.');const expires=Date.parse(connection.access_token_expires_at||'');if(Number.isFinite(expires)&&expires>Date.now()+120000)return{connection,accessToken:decryptJiraToken(connection.access_token_encrypted,jc.encryptionSecret)};if(!connection.refresh_token_encrypted)throw Object.assign(new Error('Jira access expired. Reconnect Jira.'),{status:401});const tokens=await refreshJiraTokens(decryptJiraToken(connection.refresh_token_encrypted,jc.encryptionSecret));const updated=await saveJiraConnection(c,connection.account_id,{access_token_encrypted:encryptJiraToken(tokens.access_token,jc.encryptionSecret),refresh_token_encrypted:tokens.refresh_token?encryptJiraToken(tokens.refresh_token,jc.encryptionSecret):connection.refresh_token_encrypted,access_token_expires_at:jiraTokenExpiry(tokens),scopes:clean(tokens.scope).split(/\s+/).filter(Boolean),status:'connected',last_sync_error:null});return{connection:updated,accessToken:tokens.access_token}}
...
async function loadJiraDashboard(c,accountId){const{connection,accessToken}=await jiraConnectionAccess(c,accountId),warnings=[],load=async(label,fn)=>{try{return await fn()}catch(error){warnings.push(`${label}: ${error.message}`);return[]}};const[projects,issues]=await Promise.all([load('projects',()=>listJiraProjects(accessToken,connection.cloud_id)),load('issues',()=>listJiraIssues(accessToken,connection.cloud_id))]);const synced=await saveJiraConnection(c,accountId,{last_synced_at:new Date().toISOString(),last_sync_error:warnings.length?warnings.join(' | '):null,status:'connected'});return{connection:publicJiraConnection(synced),projects,issues,warnings}}

const ZOOM_SELECT='id,account_id,zoom_user_id,zoom_account_id,connected_email,connected_name,user_type,timezone,access_token_encrypted,refresh_token_encrypted,access_token_expires_at,scopes,status,last_synced_at,last_sync_error,created_at,updated_at';
async function getZoomConnection(c,accountId){const rows=await db(c,`zoom_connections?select=${ZOOM_SELECT}&account_id=eq.${encodeURIComponent(accountId)}&limit=1`);return Array.isArray(rows)?rows[0]||null:null}
function publicZoomConnection(row){return row?{connected:row.status==='connected',zoomUserId:row.zoom_user_id||'',zoomAccountId:row.zoom_account_id||'',connectedEmail:row.connected_email||'',connectedName:row.connected_name||'',userType:Number(row.user_type||0),timezone:row.timezone||'',scopes:Array.isArray(row.scopes)?row.scopes:[],status:row.status||'connected',lastSyncedAt:row.last_synced_at||null,lastSyncError:row.last_sync_error||'',updatedAt:row.updated_at||null}:{connected:false,zoomUserId:'',zoomAccountId:'',connectedEmail:'',connectedName:'',userType:0,timezone:'',scopes:[],status:'disconnected',lastSyncedAt:null,lastSyncError:''}}
async function saveZoomConnection(c,accountId,patch){const existing=await getZoomConnection(c,accountId),now=new Date().toISOString();if(existing){const rows=await db(c,`zoom_connections?id=eq.${encodeURIComponent(existing.id)}&select=${ZOOM_SELECT}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({...patch,updated_at:now})});return Array.isArray(rows)?rows[0]||existing:existing}const rows=await db(c,`zoom_connections?select=${ZOOM_SELECT}`,{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({account_id:accountId,status:'connected',...patch,created_at:now,updated_at:now})});return Array.isArray(rows)?rows[0]||null:null}
async function deleteZoomConnection(c,accountId){await db(c,`zoom_connections?account_id=eq.${encodeURIComponent(accountId)}`,{method:'DELETE'})}
async function ensureZoomAccess(c,connection){const zc=zoomConfig();if(!zc)throw new Error('Zoom is not configured.');const expires=Date.parse(connection.access_token_expires_at||'');if(Number.isFinite(expires)&&expires>Date.now()+120000)return{connection,accessToken:decryptZoomToken(connection.access_token_encrypted,zc.encryptionSecret)};if(!connection.refresh_token_encrypted)throw Object.assign(new Error('Zoom access expired. Reconnect Zoom.'),{status:401});const tokens=await refreshZoomTokens(decryptZoomToken(connection.refresh_token_encrypted,zc.encryptionSecret));const updated=await saveZoomConnection(c,connection.account_id,{access_token_encrypted:encryptZoomToken(tokens.access_token,zc.encryptionSecret),refresh_token_encrypted:tokens.refresh_token?encryptZoomToken(tokens.refresh_token,zc.encryptionSecret):connection.refresh_token_encrypted,access_token_expires_at:zoomTokenExpiry(tokens),scopes:clean(tokens.scope).split(/\s+/).filter(Boolean),status:'connected',last_sync_error:null});return{connection:updated,accessToken:tokens.access_token}}
...
async function monitorSystemsSource(c,accountId){
  const definitions=[
    ...
    ['GHL CRM',getGhlConnection,publicGhlConnection],
    ...
    ['Zoom',getZoomConnection,publicZoomConnection],
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='GET'){
      ...
      if(action==='jira_connect'){if(!session)return json(res,401,{error:'Sign in before connecting Jira.'});if(!jiraConfig())return json(res,503,{error:'Jira environment variables are not configured.'});return json(res,200,{authorizationUrl:createJiraAuthorizationUrl(session.accountId)})}
    ...
    if(bodyAction==='jira_callback'){if(!session)return json(res,401,{error:'Your Creative Creatures login expired. Sign in again and reconnect Jira.'});const code=clean(b.code),state=clean(b.state);if(!code||!state)return json(res,422,{error:'Jira did not return the required authorization values.'});if(!verifyJiraOAuthState(state,session.accountId))return json(res,403,{error:'Jira authorization state is invalid or expired.'});const jc=jiraConfig(),tokens=await exchangeJiraCode(code),resources=await listJiraResources(tokens.access_token),resource=resources[0];if(!resource)return json(res,409,{error:'No Jira Cloud site was granted to this authorization.'});let user={};try{user=await getJiraCurrentUser(tokens.access_token,resource.id)}catch{}const saved=await saveJiraConnection(c,session.accountId,{cloud_id:resource.id,site_url:resource.url,site_name:resource.name,connected_account_id:clean(user.accountId),connected_name:clean(user.displayName),connected_email:clean(user.emailAddress),access_token_encrypted:encryptJiraToken(tokens.access_token,jc.encryptionSecret),refresh_token_encrypted:tokens.refresh_token?encryptJiraToken(tokens.refresh_token,jc.encryptionSecret):null,access_token_expires_at:jiraTokenExpiry(tokens),scopes:clean(tokens.scope).split(/\s+/).filter(Boolean),resources,status:'connected',last_sync_error:null});return json(res,200,{connected:true,connection:publicJiraConnection(saved)})}
    if(bodyAction==='jira_dashboard'||bodyAction==='jira_sync'){if(!session)return json(res,401,{error:'Sign in before viewing Jira data.'});return json(res,200,{success:true,...await loadJiraDashboard(c,session.accountId)})}
    if(bodyAction==='jira_issue_types'){if(!session)return json(res,401,{error:'Sign in before viewing Jira issue types.'});const{connection,accessToken}=await jiraConnectionAccess(c,session.accountId);return json(res,200,{issueTypes:await listJiraIssueTypes(accessToken,connection.cloud_id,clean(b.projectKey))})}
    if(bodyAction==='jira_save_issue'){if(!session)return json(res,401,{error:'Sign in before changing Jira issues.'});const{connection,accessToken}=await jiraConnectionAccess(c,session.accountId);return json(res,200,{success:true,issue:await saveJiraIssue(accessToken,connection.cloud_id,b.issue||{})})}
    ...
    if(bodyAction==='ghl_disconnect'){if(!session)return json(res,401,{error:'Sign in before disconnecting GHL CRM.'});await deleteGhlConnection(c,session.accountId);return json(res,200,{success:true,connection:publicGhlConnection(null)})}
    ...
    if(bodyAction==='zoom_meeting'){if(!session)return json(res,401,{error:'Sign in before loading a Zoom meeting.'});const meetingId=clean(b.meetingId);if(!meetingId)return json(res,400,{error:'Meeting ID is required.'});const{accessToken}=await zoomConnectionAccess(c,session.accountId);return json(res,200,{meeting:await getZoomMeeting(accessToken,meetingId)})}
