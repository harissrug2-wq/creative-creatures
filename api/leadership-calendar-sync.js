...
async function syncEvent(config, connection, team, event) {
  ...
  if (Array.isArray(existing) && existing[0]) {
    if (existing[0].source_updated_at === sourceUpdatedAt) return 'unchanged';
...
export default async function handler(req, res) {
  ...
  if (!config || !googleCalendarConfig()) {
    return json(res, 500, { error: 'Calendar sync environment variables are incomplete.' });
  }
  ...
  try {
    ...
    return json(res, 200, { ok: true, range, ...totals, errors });
  } catch (error) {
    console.error('Leadership calendar cron failed:', error);
    return json(res, 500, { error: 'Leadership calendar synchronization failed.' });
  }
}
