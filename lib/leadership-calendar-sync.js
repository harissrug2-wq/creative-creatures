...
import { fetchGoogleMeetTranscript, normalizeGoogleMeetCode } from '../lib/google-meet.js';
import { attachMeetingTranscript, processMeetingTranscript }
...
function eventHasEnded(event) {
  const endedAt = Date.parse(clean(event?.end?.dateTime || event?.end?.date));
  return Number.isFinite(endedAt) && endedAt < Date.now() - 5 * 60 * 1000;
}

function transcriptContext(config, accountId, meeting) {
  return {
    config, request: db, accountId, meetingId: meeting.id, meetingDate: meeting.meeting_date
  };
}
...
async function syncGoogleMeetTranscript(config, connection, accessToken, event) {
  const meetingUrl = googleMeetUrl(event);
  const meetingCode = normalizeGoogleMeetCode(meetingUrl);
  if (!meetingCode || !eventHasEnded(event)) return 'skipped';
  const params = new URLSearchParams({
    select: 'id,meeting_date,starts_at,ends_at,transcript_provider,transcript_status,transcript_external_id',
    account_id: `eq.${connection.account_id}`,
    calendar_event_id: `eq.${event.id}`,
    limit: '1'
  });
  const rows = await db(config, `leadership_meetings?${params.toString()}`);
  const meeting = Array.isArray(rows) ? rows[0] : null;
  if (!meeting) return 'skipped';
  const context = transcriptContext(config, connection.account_id, meeting);
  if (meeting.transcript_provider && !['none', 'google_meet'].includes(meeting.transcript_provider)) {
    return 'unchanged';
  }
  const alreadyFetched = meeting.transcript_provider === 'google_meet'
    && clean(meeting.transcript_external_id);
  if (alreadyFetched && ['processed', 'applied'].includes(meeting.transcript_status)) {
    return 'unchanged';
  }
  if (alreadyFetched) {
  ...
  const payload = await fetchGoogleMeetTranscript({
    accessToken,
    meetingCode,
    startsAt: meeting.starts_at || event.start?.dateTime,
    endsAt: meeting.ends_at || event.end?.dateTime
  });
  await attachMeetingTranscript({ ...context, body: payload });
  await processMeetingTranscript(context);
  return 'processed';
}

async function trySyncGoogleMeetTranscript(config, connection, accessToken, event) {
  try {
    return { result: await syncGoogleMeetTranscript(config, connection, accessToken, event) };
  } catch (error) {
    if ([404, 409].includes(error?.status)) return { result: 'pending' };
    return {
      result: 'failed',
      error: clean(error?.message || 'Google Meet transcript sync failed.').slice(0, 300)
    };
  }
}
...
async function syncConnection(config, connection, range) {
  ...
  try {
    ...
    const counts = {
      created: 0, updated: 0, unchanged: 0, skipped: 0, matched: matches.length,
      transcriptsProcessed: 0, transcriptsPending: 0, transcriptsFailed: 0
    };
    const transcriptErrors = [];
    for (const event of matches) {
      const result = await syncEvent(config, access.connection, team, event);
      counts[result] += 1;
      if (!eventHasEnded(event) || !googleMeetUrl(event)) continue;
      const transcript = await trySyncGoogleMeetTranscript(
        config, access.connection, access.accessToken, event
      );
      if (transcript.result === 'processed') counts.transcriptsProcessed += 1;
      if (transcript.result === 'pending') counts.transcriptsPending += 1;
      if (transcript.result === 'failed') counts.transcriptsFailed += 1;
      if (transcript.error) transcriptErrors.push(transcript.error);
    }
    await patchConnection(config, access.connection, { status: 'connected', last_error: null });
    return { ...counts, transcriptErrors };
  } catch (error) {
    ...
    for (const connection of connections || []) {
      try {
        ...
        for (const key of ['matched', 'created', 'updated', 'unchanged', 'skipped',
          'transcriptsProcessed', 'transcriptsPending', 'transcriptsFailed']) {
          totals[key] += result[key] || 0;
        }
        for (const message of result.transcriptErrors || []) {
          errors.push({ accountId: connection.account_id, error: message });
        }
      } catch (error) {
        totals.failed += 1;
