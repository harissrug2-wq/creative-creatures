...
export function ghlConfig(){const clientId=clean(process.env.GHL_CLIENT_ID),clientSecret=clean(process.env.GHL_CLIENT_SECRET),redirectUri=clean(process.env.GHL_REDIRECT_URI),encryptionSecret=clean(process.env.GHL_TOKEN_ENCRYPTION_KEY)||accountSessionSecret();return clientId&&clientSecret&&redirectUri&&encryptionSecret?{clientId,clientSecret,redirectUri,encryptionSecret,scopes:SCOPES}:null}
...
async function tokenRequest(values){const c=ghlConfig();if(!c)throw new Error('GHL CRM is not configured.');const response=await fetch(TOKEN_URL,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/x-www-form-urlencoded',Version:'v3'},body:new URLSearchParams({client_id:c.clientId,client_secret:c.clientSecret,...values})}),payload=await response.json().catch(()=>({}));if(!response.ok||!payload.access_token){const e=new Error(payload.error_description||payload.message||payload.error||'CRM token request failed.');e.status=response.status||400;e.payload=payload;throw e}return payload}
...
export const ghlTokenExpiry=tokens=>new Date(Date.now()+(Number(tokens.expires_in)||86399)*1000).toISOString();
