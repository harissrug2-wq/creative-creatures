...
async function request(accessToken,path,{method='GET',query,body,version='2021-07-28'}={}){const suffix=query?`?${new URLSearchParams(Object.entries(query).filter(([,v])=>v!==undefined&&v!==null&&v!=='')).toString()}`:'';const response=await fetch(`${API_URL}${path}${suffix}`,{method,headers:{Accept:'application/json',Authorization:`Bearer ${accessToken}`,Version:version,...(body===undefined?{}:{'Content-Type':'application/json'})},body:body===undefined?undefined:JSON.stringify(body)}),text=await response.text();let payload={};try{payload=text?JSON.parse(text):{}}catch{}if(!response.ok){const detail=clean(payload.message||payload.error_description||payload.error||payload?.errors?.[0]?.message);const e=new Error(detail||`CRM API request failed (${response.status}).`);e.status=response.status||400;e.payload=payload;throw e}return payload}
...
export async function listGhlPipelines(accessToken,locationId){const p=await request(accessToken,'/opportunities/pipelines',{query:{locationId},version:'v3'});return Array.isArray(p.pipelines)?p.pipelines:[]}
export async function listGhlOpportunities(accessToken,locationId,pipelineId=''){const p=await request(accessToken,'/opportunities/search',{query:{locationId,pipelineId,limit:100},version:'v3'});return Array.isArray(p.opportunities)?p.opportunities:[]}
export async function saveGhlOpportunity(accessToken,locationId,input={}){const id=clean(input.id),name=clean(input.name).slice(0,240);if(!name)throw Object.assign(new Error('CRM opportunity name is required.'),{status:422});const body={locationId:validId(locationId,'location'),pipelineId:validId(input.pipelineId,'pipeline'),pipelineStageId:validId(input.pipelineStageId,'pipeline stage'),name,status:['open','won','lost','abandoned'].includes(clean(input.status).toLowerCase())?clean(input.status).toLowerCase():'open',contactId:validId(input.contactId,'contact'),monetaryValue:Number(input.monetaryValue)||0};const p=await request(accessToken,id?`/opportunities/${validId(id,'opportunity')}`:'/opportunities/',{method:id?'PUT':'POST',body,version:'v3'});return p.opportunity||p}
export async function archiveGhlOpportunity(accessToken,id){await request(accessToken,`/opportunities/${validId(id,'opportunity')}`,{method:'DELETE',version:'v3'});return true}
