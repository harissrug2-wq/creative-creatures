...
export function ghlConfig(){
  const rawClientId=clean(process.env.GHL_CLIENT_ID);
  const clientId=rawClientId.includes('-')?rawClientId.split('-')[0]:rawClientId;
  ...
  return clientId&&clientSecret&&redirectUri&&encryptionSecret?{clientId,rawClientId,clientSecret,redirectUri,encryptionSecret,scopes:SCOPES}:null;
}
