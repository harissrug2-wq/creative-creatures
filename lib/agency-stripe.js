...
async function callback(req, res, agency, cfg) {
  ...
  // The OAuth state is stored server-side as a one-time hash bound to the signed-in
  // agency and Stripe environment. Do not require the helper cookie here: some
  // browsers drop that cookie during Stripe's cross-site onboarding redirect.
  if (!/^[a-f0-9]{64}$/.test(state)) throw fail(403, 'Stripe authorization state is invalid. Start the connection again.');
  ...
  res.statusCode = 303; res.setHeader('Location', `${cfg.origin}/integrations/?stripe=connected`); res.end();
}


export async function readAgencyStripeContext(agency){
  const cfg=connectConfig();
  const row=(await connectDb(`${connectionPath(agency,cfg)}&limit=1`))?.[0];
  if(row?.status!=='connected'||!accountId(row.stripe_account_id))return{connected:false,configured:true,environment:cfg.live?'production':'test'};
  const connected=row.stripe_account_id;
  const [customersResult,invoicesResult]=await Promise.allSettled([
    stripeRequest(cfg,'customers?limit=25',{connected}),
    stripeRequest(cfg,'invoices?limit=25',{connected})
  ]);
  const warnings=[];
  const customers=customersResult.status==='fulfilled'?(customersResult.value?.data||[]).map(customerView):(warnings.push(customersResult.reason?.message||'Customers could not be loaded.'),[]);
  const invoices=invoicesResult.status==='fulfilled'?(invoicesResult.value?.data||[]).map(invoiceView):(warnings.push(invoicesResult.reason?.message||'Invoices could not be loaded.'),[]);
  return{connected:true,configured:true,environment:cfg.live?'production':'test',displayName:row.display_name||'',stripeAccountId:connected,customers,invoices,warnings};
}
