...
const PRODUCTION_REDIRECT_URI = 'https://app.creativecreatures.org/integrations/quickbooks/callback/';

function canonicalQuickBooksRedirect(value, environment = 'sandbox') {
  // Production has one canonical callback. Do not let an old/mistyped Vercel
  // variable change the redirect_uri Intuit sees for the live app.
  if (environment === 'production') return PRODUCTION_REDIRECT_URI;
  const raw = clean(value);
  try {
    const url = new URL(raw);
    if (url.hostname.toLowerCase() === 'app.creativecreatures.org') {
      url.protocol = 'https:';
      url.pathname = '/integrations/quickbooks/callback/';
      url.search = '';
      url.hash = '';
      return url.toString();
    }
...
export function quickBooksConfig() {
  ...
  const redirectUri = canonicalQuickBooksRedirect(process.env.QUICKBOOKS_REDIRECT_URI, environment);
...
// Never attach provider payloads, credentials or financial values to errors.
function providerError(response, payload, operation) {
  const code = String(payload?.error || payload?.Fault?.Error?.[0]?.code || 'QUICKBOOKS_RESPONSE_ERROR').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 80);
  const intuitTid = String(response.headers.get('intuit_tid') || '').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 128);
  const error = Object.assign(new Error(`QuickBooks ${operation} failed. Please retry or contact support.${intuitTid ? ` Reference: ${intuitTid}.` : ''}`), {
    status: response.ok ? 502 : response.status, code, intuitTid, provider: 'quickbooks'
  });
  console.error('quickbooks_request_failed', {operation, status:error.status, code, intuitTid});
  return error;
}

export async function withQuickBooksRecovery(access, refresh, operation) {
  try { return await operation(access); }
  catch (error) {
    if (error.status !== 401) throw error;
    const renewed = await refresh();
    try { return await operation(renewed); }
    catch (retryError) {
      if (retryError.status === 401) retryError.code = 'QUICKBOOKS_RECONNECT_REQUIRED';
      throw retryError;
    }
  }
}
...
async function tokenRequest(body) {
  ...
  if (!response.ok || !payload?.access_token) {
    throw providerError(response, payload, 'authorization');
  }
  return payload;
}
...
export async function quickBooksApiRequest({ realmId, accessToken, path, query }) {
  ...
  const payload = await response.json().catch(() => ({}));
  if (!response.ok || !payload || !Object.keys(payload).length || payload.Fault) {
    throw providerError(response, payload, 'report request');
  }
  return payload;
}
...
export async function fetchProfitLossEvidence({ realmId, accessToken }) {
  ...
  return {
    ...
    operatingExpensesTTM: currentExpenses,
...
export async function fetchBalanceSheetEvidence({ realmId, accessToken }) {
  ...
  const accountsReceivable = reportScalar(report, [
    label => /^total accounts receivable(?: a r)?$/.test(label),
    label => /^accounts receivable(?: a r)?$/.test(label)
  ]);
