...
function canonicalQuickBooksRedirect(value) {
  const raw = clean(value);
  try {
    const url = new URL(raw);
    // Creative Creatures has one production callback route. Normalize legacy
    // variants so Intuit always receives the exact URI registered for the app.
...
export function quickBooksConfig() {
  ...
  const redirectUri = canonicalQuickBooksRedirect(process.env.QUICKBOOKS_REDIRECT_URI);
