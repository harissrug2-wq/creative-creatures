...
export async function fetchArAgingEvidence({ realmId, accessToken }) {
  ...
  let within30 = 0, hasWithin30 = false;
  if (totals) {
    columns.forEach(({ index, title }) => {
      if (/current|1\s*-\s*30|1\s*to\s*30|0\s*-\s*30/i.test(title)) {
        const value = finite(totals.data[index]?.value);
        if (value !== null) { within30 += value; hasWithin30 = true; }
      }
    });
  }
  return {
    ...
    asOfDate: isoDate(end),
    totalAR,
    collectionRatePercent: hasWithin30 && finite(totalAR) ? percent(within30, totalAR) : null
  };
}
