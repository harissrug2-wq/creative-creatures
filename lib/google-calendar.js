...
export function googleMeetConnectionStatus(row) {
  const scopes = clean(row?.scope).split(/\s+/).filter(Boolean);
  const missingScopes = ['https://www.googleapis.com/auth/meetings.space.readonly']
    .filter(scope => !scopes.includes(scope));
  const reconnectRequired = Boolean(row) && (missingScopes.length > 0 || row.status === 'error');
  const connected = row?.status === 'connected' && !reconnectRequired;
  return {
    connected,
    reconnectRequired,
    transcriptPermissionsGranted: Boolean(row) && missingScopes.length === 0,
    missingScopes,
    scopes,
    status: !row ? 'disconnected' : reconnectRequired ? 'reconnect_required' : connected ? 'connected' : row.status,
