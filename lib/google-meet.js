...
export async function fetchGoogleMeetTranscript({ accessToken, meetingCode, startsAt, endsAt }) {
  const code = normalizeGoogleMeetCode(meetingCode);
  const records = await listGoogleMeetConferenceRecords({ accessToken, meetingCode: code });
  const targetTime = Date.parse(endsAt || startsAt || '');
  const completed = [...records].filter(item => item?.endTime);
  const record = Number.isFinite(targetTime)
    ? completed.sort((a, b) => (
      Math.abs(Date.parse(a.endTime || a.startTime || 0) - targetTime)
      - Math.abs(Date.parse(b.endTime || b.startTime || 0) - targetTime)
    ))[0]
    : completed.sort((a, b) => Date.parse(b.endTime || 0) - Date.parse(a.endTime || 0))[0];
  if (!record) throw serviceError('No completed Google Meet conference was found.', 404, 'GOOGLE_MEET_CONFERENCE_NOT_FOUND');
  if (Number.isFinite(targetTime)
    && Math.abs(Date.parse(record.endTime || record.startTime || 0) - targetTime) > 12 * 60 * 60 * 1000) {
    throw serviceError('No Google Meet conference matched this meeting occurrence.', 404, 'GOOGLE_MEET_OCCURRENCE_NOT_FOUND');
  }
