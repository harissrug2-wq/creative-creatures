...
export async function generateAnswer(context,history,message,fetcher=fetch){
 ...
 const r=await fetcher('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},signal:AbortSignal.timeout(45000),body:JSON.stringify({
  model:process.env.OPENAI_MODEL||'gpt-5-mini',instructions:instructions(),
  input:[...boundedCreatureHistory(history),{role:'user',content:JSON.stringify({context,question:message})}],
  text:{format:{type:'json_schema',name:'creature_answer',strict:true,schema:{type:'object',properties:{in_scope:{type:'boolean'},answer:{type:'string'}},required:['in_scope','answer'],additionalProperties:false}}},max_output_tokens:1200
 })});
 const p=await r.json().catch(()=>null);if(!r.ok)throw fail(502,'Ask Creature could not respond. Please retry.');
 const text=(p?.output||[]).flatMap(o=>o.content||[]).filter(c=>c.type==='output_text').map(c=>c.text).join('');
 let result;try{result=JSON.parse(text);}catch{throw fail(502,'Ask Creature returned an incomplete answer. Please retry.');}
