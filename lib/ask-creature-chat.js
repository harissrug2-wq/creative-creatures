...
export async function generateAnswer(context,history,message,fetcher=fetch){
 ...
 const request={
  store:false,model,instructions:instructions()+' Keep answers concise, usually under 250 words.',
...
export async function sendChat(db,account,actor,input,{answer=generateAnswer,liveContext=null}={}){
 ...
 if(!response){
  ...
  response=await answer({agency:account.agency_name||account.name,actorRole:actor.role,departments:actor.departments||[],page:topic,currentTime:new Date().toISOString(),diagnostic:details?compactCreatureData(details.diagnostic_state||{},5000):undefined,report:details?compactCreatureData(details.report_data||{},5000):undefined,connectedData:liveContext?compactCreatureData(liveContext,28000):undefined},history,message);
 }
