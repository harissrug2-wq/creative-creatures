(() => {
  ...
  document.querySelectorAll('[data-app-header]').forEach(async el => {
    ...
    el.innerHTML = `
      ...
      <nav class="mobile-nav-panel">${mobileNav.map(([label,href,enabled,key]) => `<a href="${href}" data-workspace-feature="${key}" data-flow-locked="${enabled?'0':'1'}" hidden class="${active===key?'active ':''}${enabled?'':'nav-disabled'}">${label}</a>`).join('')}<a href="/ownership/">Ownership & Partners</a><a href="/account/upgrade/" data-account-upgrade hidden>Upgrade account</a>${identity ? `<button class="mobile-signout" type="button">${icon('logout')}Sign out ${esc(displayName || agencyName)}</button>` : ''}<button class="mobile-ask-creature" type="button" hidden><img src="/brand/creature-icon.png" class="cc-ask-logo-icon" alt="" style="width:16px;height:16px;object-fit:contain;margin-right:6px;vertical-align:middle;">Ask Creature</button></nav>
      ${status}
      ${identity ? `<div class="top-account-menu" hidden><strong>${esc(displayName || agencyName)}</strong><span>${esc(account?.email || '')}</span><a href="/ownership/">Ownership & Partners</a><a href="/account/upgrade/" data-account-upgrade hidden>Change account type</a><button type="button">${icon('logout')}Sign out</button></div>` : ''}`;
