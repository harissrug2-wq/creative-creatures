(() => {
  ...
  function renderUsers(){
    shell(`<div class="users-head"><div><h1 class="page-title">Manage Users</h1><p class="page-subtitle">Create team logins and limit each person to the departments they need.</p></div><button class="primary-btn" id="newWorkspaceUser">＋ New User</button></div><section class="card users-card" id="workspaceUsers"><div class="empty-state">Loading account users…</div></section><div class="workspace-user-modal" id="workspaceUserModal" hidden><form class="workspace-user-form"><div class="workspace-user-head"><h2 id="workspaceUserTitle">New user</h2><button type="button" id="workspaceUserClose">×</button></div><input type="hidden" name="id"><label>Full name<input name="name" required maxlength="120"></label><label>Email<input name="email" type="email" required></label><label>Temporary password<input name="password" type="password" minlength="10" placeholder="At least 10 characters"></label><fieldset><legend>Department access</legend><div id="workspaceDepartments" class="workspace-departments"></div></fieldset><label>Status<select name="status"><option value="active">Active</option><option value="disabled">Disabled</option></select></label><p class="workspace-user-error" hidden></p><div class="modal-actions"><button type="button" class="secondary-btn" id="workspaceUserCancel">Cancel</button><button class="primary-btn">Save user</button></div></form></div>`,'Manage Users');
    const load=()=>new Promise((resolve,reject)=>{if(window.CCWorkspace)return resolve(window.CCWorkspace);let script=document.querySelector('script[data-cc-workspace]');if(!script){script=document.createElement('script');script.src='/shared/workspace-access.js';script.dataset.ccWorkspace='1';document.head.appendChild(script)}script.addEventListener('load',()=>resolve(window.CCWorkspace),{once:true});script.addEventListener('error',reject,{once:true})});
    load().then(async workspace=>{let data;try{data=await workspace.request('workspace_users')}catch(error){document.querySelector('#workspaceUsers').innerHTML=`<div class="empty-state">${safe(error.message)}</div>`;document.querySelector('#newWorkspaceUser').hidden=true;return}const card=document.querySelector('#workspaceUsers'),modal=document.querySelector('#workspaceUserModal'),form=modal.querySelector('form'),errorEl=form.querySelector('.workspace-user-error');const labels={leadership:'Leadership',marketing:'Marketing',sales:'Sales',billing:'Billing',onboarding:'Onboarding','service-delivery':'Service Delivery','client-success':'Client Success','talent-acquisition':'Talent Acquisition',finance:'Finance',communication:'Communication',systems:'Systems / IT',sops:'SOPs'};document.querySelector('#workspaceDepartments').innerHTML=data.departments.map(d=>`<label><input type="checkbox" name="departments" value="${d}"> ${labels[d]||d}</label>`).join('');const render=()=>{card.innerHTML=`<div class="card-titlebar">${data.users.length+1} account user${data.users.length?'s':''}</div><div class="user-row"><span class="user-avatar">${ico('person')}</span><div class="user-info"><div class="user-line"><strong>${safe(data.owner.name)}</strong><span class="role">Owner</span></div><div class="user-email">${safe(data.owner.email)} · Full account access</div></div></div>${data.users.map(u=>`<div class="user-row"><span class="user-avatar">${ico('person')}</span><div class="user-info"><div class="user-line"><strong>${safe(u.name)}</strong><span class="role">${safe(u.status)}</span></div><div class="user-email">${safe(u.email)} · ${u.departments.map(d=>labels[d]||d).join(', ')}</div></div><div class="user-actions"><button class="secondary-btn" data-edit-user="${u.id}">Edit</button><button class="danger-btn" data-remove-user="${u.id}">Remove</button></div></div>`).join('')}`};const open=u=>{form.reset();form.id.value=u?.id||'';form.name.value=u?.name||'';form.email.value=u?.email||'';form.email.disabled=Boolean(u);form.status.value=u?.status||'active';form.password.required=!u;document.querySelector('#workspaceUserTitle').textContent=u?'Edit user':'New user';form.querySelectorAll('[name=departments]').forEach(x=>x.checked=Boolean(u?.departments.includes(x.value)));errorEl.hidden=true;modal.hidden=false;document.body.classList.add('cc-modal-open')};const close=()=>{modal.hidden=true;document.body.classList.remove('cc-modal-open')};render();document.querySelector('#newWorkspaceUser').onclick=()=>open();if(new URLSearchParams(location.search).get('invite')==='1')open();document.querySelector('#workspaceUserClose').onclick=close;document.querySelector('#workspaceUserCancel').onclick=close;card.onclick=async e=>{const edit=e.target.closest('[data-edit-user]'),remove=e.target.closest('[data-remove-user]');if(edit){open(data.users.find(u=>u.id===edit.dataset.editUser));return}if(remove&&confirm('Remove this user from the agency account?')){await workspace.request('',{method:'POST',body:{action:'workspace_remove_user',id:remove.dataset.removeUser}});data.users=data.users.filter(u=>u.id!==remove.dataset.removeUser);render()}};form.onsubmit=async e=>{e.preventDefault();const payload={id:form.id.value,name:form.name.value,email:form.email.value,password:form.password.value,status:form.status.value,departments:[...form.querySelectorAll('[name=departments]:checked')].map(x=>x.value)};try{const result=await workspace.request('',{method:'POST',body:{action:payload.id?'workspace_update_user':'workspace_invite_user',...payload}});const index=data.users.findIndex(u=>u.id===result.user.id);if(index>=0)data.users[index]=result.user;else data.users.push(result.user);render();close();showToast('User access saved.')}catch(err){errorEl.textContent=err.message;errorEl.hidden=false}}}).catch(error=>{document.querySelector('#workspaceUsers').innerHTML=`<div class="empty-state">${safe(error.message)}</div>`});
  }
