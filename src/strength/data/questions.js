...
export function computeResults(answers, scaleTestAnswer) {
  ...
  // The scoring PDF treats the questionnaire as an initial hypothesis. Without
  // connected operational evidence, confidence should not read as fully verified.
  let validationStatus = 'Needs Validation';
  let confidenceScore = 70;
  if (mismatch !== null) {
    if (mismatch > 30) {
      validationStatus = 'Significant Contradiction';
      confidenceScore = Math.max(35, Math.round(70 - (mismatch - 30)));
    } else if (mismatch > 15) {
      validationStatus = 'Needs Validation';
      confidenceScore = Math.max(50, Math.round(70 - (mismatch - 15) * 0.8));
    }
  }
